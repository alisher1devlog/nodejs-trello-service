...
const taskModel = {
    ...
    getById: async (board_id, task_id) => {
        const result = await pool.query(
    ...
    create: async (boardId, taskData) => {
        ...
        const result = await pool.query(
            `INSERT INTO tasks (title, "order", description, user_id, board_id, column_id) 
            VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [title, order, description || null, userId || null, boardId, columnId || null]
        );
        return result.rows[0];
    },
